...
#функция, лемматизирующая входящий список текстов (возвращает лемматизированный список того же размера)
def text_coll_lemmatizer_replacement (texts_list, applicants_list, responders_list, stoplist):
    t_lemm = []    
    #для лемматизации объединим тексты из списка в один с разделителем (перенос строки):
    if  applicants_list != [] and responders_list != []:
        txts_for_lemm = '\n'.join(texts_without_line_break(texts_list))        
        applicants_for_lemm = '\n'.join(texts_without_line_break(applicants_list))
        responders_for_lemm = '\n'.join(texts_without_line_break(responders_list))
        lemm_txt = texts_lemm(txts_for_lemm)
        lemm_app = texts_lemm(applicants_for_lemm)
        lemm_res = texts_lemm(responders_for_lemm)
        
        texts_for_handling = list(zip(lemm_txt.split('\n'), lemm_app.split('\n'), 
                                      lemm_res.split('\n')))
                
        for item in texts_for_handling:
            if len(item[0])>0:
                txt = re.sub(txt_handling(item[1]), ' истец ', txt_handling(item[0]))
                txt = re.sub(txt_handling(item[2]), ' ответчик ', txt)                             
                t_lemm.append(txt)
    else:
        txts_for_lemm = '\n'.join(texts_without_line_break(texts_list))
        lemm_txts = texts_lemm(txts_for_lemm)
        for tx in lemm_txts.split('\n'):
            tx = txt_handling(tx)
            if tx!='':
                t_lemm.append(tx)
        t_lemm = [[word for word in txt.split() if word not in stoplist] for txt in t_lemm]        
    return t_lemm #, texts_for_handling#, lemm_txt, lemm_app, lemm_res

def csv_utf2win_convertor (file_for_convert_rout, file_name):
    df_for_conv = pd.read_csv(os.path.join(file_for_convert_rout, file_name))
    #df_for_conv['2'].str.replace('[^\w\s]', '')
    new_fn = re.sub('.csv', '_win.csv', file_name)
    df_for_conv.to_csv(os.path.join(file_for_convert_rout, new_fn), sep = ',', encoding = '1251')
    return None
