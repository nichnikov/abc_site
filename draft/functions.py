...
def new_nn_learning(model_rout, data_rout, dict_rout, ds, min_class_examples=100, nn_epochs=5):
    #отберем те классы примеров, в которых больше n (например, n=100) текстов:
    try:
        txts, lbls = data_for_learning_selection(ds, each_class_examples=min_class_examples)
    except:
    ...
    try:
        #сначала попытаемся найти и дообучить существующую модель Word2Vec
        w2v_model = models.Word2Vec.load(os.path.join(model_rout, 'w2v_model'))
        print('дообучим word2vec модель')
        #обновим словарь модели:
        new_sents = [tx.split() for tx in lemm_txts]
        w2v_model.train(new_sents, total_examples = w2v_model.corpus_count, epochs=10)
        print('дообучили word2vec модель')
    except:
        #если модели не существует, то создаем word2vec модель
        #зададим модель
        print('создаем новую модель w2v')
        #инициилизируем модель
        w2v_model = models.Word2Vec(min_count=30, iter=25, size=150, window=5,  workers=7)
        sents_for_model = [tx.split() for tx in lemm_txts]
        #подготовим словарь
        w2v_model.build_vocab(sents_for_model)
        #обучим модель:
        w2v_model.train(sents_for_model, total_examples = w2v_model.corpus_count, epochs=10)
    ...
    #сначала попытаемся найти существующий токенайзер и использовать его:
    try:
        with open(os.path.join(model_rout, 'tokenizer.pickle'), 'rb') as file:
            tokenizer = pickle.load(file)
        #обновим токенайзер:
        tokenizer.fit_on_texts(texts_for_working)
        print('токенайзер обновился')
    except:
        #если готового токенайзера нет, приготовим его:
        tokenizer = Tokenizer()
        tokenizer.fit_on_texts(texts_for_working)
    ...
    #выбор одного тега для работы с ним (остальные тексты считаются классом 0) с балансировкой или без
    #применимо к классификации "один ко многим"
    #ВАЖНО: пока не будет реализовано обращение к БД с примерами,
    #нужно на вход всегда подавать несколько классов, на одном классе обучаться не сможет
    ...
    for tag_num in list(set(lbls)):
        ...
        model.fit([data], y, validation_split=0.2, epochs=nn_epochs, batch_size=32, shuffle=True)
...
#функция дообучения моделей, загружает готовую модель и ее дообучает
def adding_nn_lerning (model_rout, lables_nums, texts_with_lables, tokenizer, w2v_model, work_vocab, MAX_SEQUENCE_LENGTH, balance_type = True, epochs_num=5):
    for tag_num in lables_nums:
        txs0, txs1 = tag_handling(texts_with_lables, tag_num, balance = balance_type)
        texts_for_learning = txs0 + txs1
        lbs = len(txs0)*[0] + len(txs1)*[1]  
        ...
        #дообучим модель:
        nn_model.fit([data], y, validation_split=0.2, epochs=epochs_num, batch_size=32, shuffle=True)
        nn_model.save(os.path.join(model_rout, model_name))
    return None

#дообучим имеющиеся нейронные сети на новых данных:
def nn_updating_new_data(df, model_rout, MAX_SEQUENCE_LENGTH = 15, min_class_examples = 1, nn_epochs = 5):
    #отберем те классы примеров, в которых больше n (например, n=100) текстов:
    txts, lbls = data_for_learning_selection(df, each_class_examples=min_class_examples)
    ...
    adding_nn_lerning(model_rout, lables_nums, texts_with_lables, tokenizer, w2v_model, work_vocab, MAX_SEQUENCE_LENGTH, balance_type = True, epochs_num=nn_epochs)
    return None
